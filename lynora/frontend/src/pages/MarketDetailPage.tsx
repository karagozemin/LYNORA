...
import { formatMassaAmount } from '../lib/massa'

// Mock markets (same as HomePage) for fallback
...
export default function MarketDetailPage() {
  ...
  const loadMarket = async () => {
    ...
    try {
      ...
      // Try to get markets from API first
      let allMarkets: Market[] = []
      try {
        allMarkets = await getAllMarkets()
      } catch (err) {
        console.warn('Error loading markets from API, using mock markets:', err)
      }
      
      // Combine with mock markets (avoid duplicates by ID)
      ...
      const foundMarket = combinedMarkets.find(m => m.id === marketId)
  ...
  // totalUpBets and totalDownBets are in MAS (string), but totalPool might be in nanoMAS
  const upBets = parseFloat(market.totalUpBets || '0')
  const downBets = parseFloat(market.totalDownBets || '0')
  // totalPool is in nanoMAS (string), convert to MAS
  const totalPoolNano = BigInt(market.totalPool || '0')
  const totalPool = Number(totalPoolNano) / 1e9 // Convert nanoMAS to MAS
  ...
  // optionBets values are in nanoMAS (string), convert to MAS for display
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-700 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-white">{displayTotalPool.toFixed(2)} MAS</div>
          </div>
          <div className="bg-gray-700 rounded-lg p-4">
        ...
        {isQuestionAnswerMarket ? (
          <div className="mb-6">
            <div className="flex items-center justify-between text-sm text-gray-400 mb-4">
              ...
              <span className="text-white font-semibold">{totalOptionPool.toFixed(2)} MAS Total</span>
            </div>
            
            <div className="space-y-3">
              {market.options!.map((option, index) => {
                // Get amount from optionTotals (already converted to MAS)
                ...
                return (
                  <div key={index} className="border border-gray-600 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-white font-semibold">{amount.toFixed(2)} MAS</span>
                    </div>
            ...
            {(market.status === 'Locked' || market.status === 'Resolved') && market.correctAnswer && (
              <div className="mt-4 p-4 bg-green-900 bg-opacity-30 border border-green-700 rounded-lg">
                ...
                <p className="text-gray-300 text-sm mt-1">
                  Winners receive 2x their bet (max 10 MAS per winner)
                </p>
              </div>
            )}
                  ...
                  <div key={reward.id} className="flex items-center justify-between mb-2 p-3 bg-gray-800 rounded-lg">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        Reward: {formatMassaAmount(BigInt(reward.reward_amount))} MAS
                      </p>
                    </div>
                    <button
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-semibold">{upBets.toFixed(2)} MAS</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-red-400 text-lg">↓ DOWN</span>
                <span className="text-white font-semibold">{downBets.toFixed(2)} MAS</span>
              </div>
            </div>
          </div>
        )}
            ...
            {mockBets.map((bet, index) => (
              <div key={index} className="flex items-center justify-between bg-gray-700 rounded-lg p-4">
                ...
                <div className="text-right">
                  <div className="text-white font-semibold">{bet.amount} MAS</div>
